...
def _blazegraph_text_search_filter(target: _base.TextSearchTarget, value: str) -> list[str]:
    return [f"{target.variable} <http://www.bigdata.com/rdf/search#search> {_base.sparql_string(value)} ."]
...
handle_skg_if_product_filter = _base.make_product_filter_handler(_blazegraph_text_search_filter)
