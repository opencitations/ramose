...
def _get_operation(am: APIManager, dois_param: str) -> Operation:
    base = am.base_url[0]
...
class TestMultiSourceErrorHandling:
    def test_sparql_endpoint_failure_returns_502(self) -> None:
        am = _load_api_manager("test_scholarly_multi-sources.hf")
...
class TestMultiSourceUnknownStepTag:
    def test_unknown_step_tag_returns_502(self) -> None:
        am = _load_api_manager("test_scholarly_multi-sources.hf")
        op = _get_operation(am, "10.1108/jd-12-2013-0166")
...
class TestMultiSourceValuesInject:
    def test_values_inject_in_multi_source(self) -> None:
        am = _load_api_manager("test_scholarly_multi-sources.hf")
        op = _get_operation(am, "10.1108/jd-12-2013-0166")
...
class TestParseSteps:
    def _make_op(self, sources_map: dict[str, str] | None = None) -> Operation:
        op_item = {
            "url": "/test/{id}",
            "id": "str(.+)",
    ...
    def test_simple_query_no_directives(self) -> None:
        op = self._make_op()
    ...
    def test_inner_join_default(self) -> None:
        op = self._make_op()
        text = "SELECT ?a WHERE { }\n@@join ?a ?b\nSELECT ?b WHERE { }"
        steps = op._parse_steps(text, "http://ep/sparql", {})
    ...
    def test_values_empty_raises(self) -> None:
        op = self._make_op()
        text = "@@values\nSELECT ?a WHERE { }"
        with pytest.raises(ValueError, match="@@values needs at least one variable"):
            op._parse_steps(text, "http://ep/sparql", {})

    def test_values_with_colon_treated_as_variable(self) -> None:
        op = self._make_op()
        text = "@@values ?a:x\nSELECT ?a WHERE { }"
        steps = op._parse_steps(text, "http://ep/sparql", {})
    ...
    def test_join_keyword_syntax(self) -> None:
        op = self._make_op()
        text = "SELECT ?a WHERE { }\n@@join left_var=?a right_var=?b type=left\nSELECT ?b WHERE { }"
        steps = op._parse_steps(text, "http://ep/sparql", {})
        assert steps[1] == ("JOIN", "?a", "?b", "left")

    def test_join_mixed_positional_keyword(self) -> None:
        op = self._make_op()
        text = "SELECT ?a WHERE { }\n@@join ?a right_var=?b type=left\nSELECT ?b WHERE { }"
        steps = op._parse_steps(text, "http://ep/sparql", {})
        assert steps[1] == ("JOIN", "?a", "?b", "left")

    def test_join_keyword_reversed_order(self) -> None:
        op = self._make_op()
...
class TestJoin:
    ...
    def test_join_normalizes_http_https(self) -> None:
        op = self._make_op()
        left = [{"br": "http://example.org/br/1", "doi": "10.1"}]
        right = [{"br": "https://example.org/br/1", "count": "3"}]
        result = op._join(left, right, "?br", "?br", "inner")  # type: ignore[arg-type]
        assert len(result) == 1
        assert result[0]["count"] == "3"

    def test_join_normalizes_trailing_slash(self) -> None:
        op = self._make_op()
        left = [{"uri": "http://example.org/x/"}]
        right = [{"uri": "http://example.org/x", "val": "y"}]
        result = op._join(left, right, "?uri", "?uri", "inner")  # type: ignore[arg-type]
        assert len(result) == 1

    def test_join_empty_right(self) -> None:
        op = self._make_op()
        left = [{"a": "1"}, {"a": "2"}]
        result = op._join(left, [], "?a", "?a", "left")  # type: ignore[arg-type]
        assert len(result) == 2

    def test_join_empty_left(self) -> None:
        op = self._make_op()
        result = op._join([], [{"a": "1"}], "?a", "?a", "inner")
        assert len(result) == 0

    def test_join_right_none_value_skipped(self) -> None:
        op = self._make_op()
    ...
    def test_join_column_conflict_creates_suffix(self) -> None:
        op = self._make_op()
        left = [{"doi": "10.1", "title": "A"}]
    ...
    def test_join_right_null_key_skipped(self) -> None:
        op = self._make_op()
        left = [{"doi": "10.1"}]
...
class TestDropColumns:
    def test_removes_specified_columns(self) -> None:
        ...
        result = Operation._drop_columns(rows, ["?b", "?c"])  # type: ignore[arg-type]
...
class TestHeaderFromFieldType:
    def test_extracts_field_names_in_order(self) -> None:
        op_item = {"field_type": "str(doi) str(qid) int(count)"}
        result = Operation._header_from_field_type(op_item, [])
        assert result == ["doi", "qid", "count"]

    def test_fallback_to_dict_keys(self) -> None:
        op_item = {}
        acc = [{"a": "1", "b": "2"}]
        result = Operation._header_from_field_type(op_item, acc)  # type: ignore[arg-type]
        assert result == ["a", "b"]

    def test_empty_acc_no_field_type(self) -> None:
        assert Operation._header_from_field_type({}, []) == []


class TestToCsvRows:
    def test_produces_header_plus_data(self) -> None:
        header = ["x", "y"]
        acc = [{"x": "1", "y": "2"}, {"x": "3", "y": "4"}]
        result = Operation._to_csv_rows(header, acc)  # type: ignore[arg-type]
...
class TestRunSparqlDicts:
    def _make_op(self, method: str = "get") -> Operation:
        op_item = {
            "url": "/test/{id}",
            "id": "str(.+)",
            "sparql": "SELECT ?x WHERE { }",
    ...
    def test_get_request(self, mock_session: MagicMock) -> None:
    ...
    @patch("ramose.operation._http_session")
    def test_post_request(self, mock_session: MagicMock) -> None:
        resp = SimpleNamespace(
            status_code=200,
            content=b"x\nval\n",
            reason="OK",
    ...
    @patch("ramose.operation._http_session")
    def test_non_200_raises(self, mock_session: MagicMock) -> None:
        resp = SimpleNamespace(
            status_code=500,
            content=b"error",
